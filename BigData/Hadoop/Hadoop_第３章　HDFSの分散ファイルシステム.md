...
**step3**：アップロード許可を獲得した後でFSDataOutputStreamインスタンスを作成し、200M目標ファイルを分割すると始めます。

　　書き込み流れが一括に書き込むことではありません。1番目のブロックを分割されて出たから、対応の格納アドレスと副本アドレスを獲得して順序で各DataNodeにデータを発送したまでは、一つの連続、完全のアップロード流れです。その初めて、次の2番目のブロックのアップロード流れを始めて進行します。つまり、下図の緑部分が一回のアップロード流れに属するものです

　　NameNodeから取得したDataNodeアドレスのリストをパイプライン（Pipeline）にして格納されます（⑥）。そのパイプライン上の節点順調で逐次に対応のDataNodeにデータを発送します。
