...
**step2**：上図には不足点があるのはクライアントがNameNodeに請求を発出してファイルの分割ブロックの格納アドレス情報を獲得する矢印を付くはずで、非常に重要の過程です。1番目のブロックを分割されて出た、対応の格納アドレスを獲得していき、副本も含まれて全てのブロックをアップロードしたまでは一つの連続、完全のアップロード流れです。その後で次の2番目のブロックのアップロード流れを始めて進行するわけです。つまり、図の下の緑部分が一回のアップロード流れに属するだけです。

**step3**：ブロック情報を獲得してFSDataOutputStreamインスタンスを作成し、読み込みのFSDataInputStreamに比べて明らかに出力の用途です。NameNodeから取得した格納情報に従って対応のDataNodeに通信を立てます。
...
　　次は正式にブロックのアップロードが開始します。読み込みと同じでDFSOutputStreamはデータを書き込む際にはデータブロックを一つ一つのパケット（packets）に分割される形式で目標のDataNodeに発送します。dn16節点へパケットを発送してdn16節点が受け取るパケットをまずメモリに格納しておきます。格納済みに次のdn57節点へパケットをアップロードします。最後のdn89節点がパケットの受け取りが済むなら逐次に上の節点に成功の確認情報を返却して、最終にクライアントのDFSOutputStreamが返却信号を受信したまで一つの完全のデータストリート伝送がありしました。残りの過程が上述の流れを繰り返すわけです。
