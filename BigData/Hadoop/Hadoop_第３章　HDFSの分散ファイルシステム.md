...
**step1**：先ず依然クライアントがDistributed FileSystemのインスタンスを作成します。このインスタンスはNameNodeに請求を発出して目標ファイルをアップロードする可否を確認します。アップロード許可を獲得した後で200M目標ファイルを分割すると始めます。

　　NameNode がユーザーのファイル書き込みRPC要求を受け取ると、まず様々なチェックを実行します。例えば、ユーザーが関連する作成権限を持っているか、またそのファイルが既に存在していないかなどを確認します。これらのチェックに合格した場合、新しいファイルが作成され、その操作がトランザクションログに記録されます。

**step2**：上図には不足点があるとはクライアントがNameNodeに請求を発出してファイルの分割ブロックの格納アドレス情報を獲得する矢印を付くはずで、非常に重要の過程です。1番目のブロックを分割されて出た、対応の格納アドレスを獲得していき、副本も含まれて全てのブロックをアップロードしたまでは一つの連続、完全のアップロード流れです。その後で次の2番目のブロックのアップロード流れを始めて進行するわけです。つまり、図の下の緑部分が一回のアップロード流れに属するだけです。

**step3**：ブロック情報を獲得してFSDataOutputStreamインスタンスを作成し、読み込みのFSDataInputStreamに比べて明らかに出力の用途です。NameNodeから取得した格納リストに従って対応のDataNodeに通信を立てます。

**step4/step5**：今は図を参考として複数のDataNode節点アドレスを返却されたことがあり、分別がdn16、dn57、dn89三つの節点です。クライアントのFSDataOutputStreamが一つ目のdn16に信号を発送してアップロード操作があると知らせます。次にdn16節点がdn57節点に知らせ、次第に次の節点に同じな操作を行います。最後のdn89節点まで請求信号を受信したら前の節点に返信することが始めます。最初の送信者FSDataOutputStreamが順調にdn16節点の返信を受け取ると一つの完全な通信チャンネルを確立します。もちろんその途中である節点が故障になったらクライアントが改めてNameNodeに新たなアドレスを返信させます。

　　次は正式にブロックのアップロードが開始します。読み込みと同じでDFSOutputStreamはデータを書き込む際にはデータブロックを一つ一つのパケット（packets）に分割される形式で目標のDataNodeに発送します。

　　dn16節点へパケットを発送してdn16節点が受け取るパケットを、まずメモリに格納しておきます。格納済みに次のdn57節点へパケットをアップロードします。最後のdn89節点がパケットの受け取りが済むなら逐次に前の節点に成功の確認情報を返却します。全てのブロックパケットを受信した限り、ブロックを永続化にして書き込みます（多分）。最終にクライアントのDFSOutputStreamが返却信号を受信したまで一つの完全のデータストリート伝送が形成されます。残りの過程が上述の流れを繰り返すだけです。

　　DFSOutputStreamは内部に別の書き込みデータ隊列（ack queue）も維持しています。この隊列の中で送信のパケットが順序で並べたと、完全に各節点がパケットを保存するのを確認したら、ack queueからそのパケットを削除します。
