...
　　書き込み流れが一括に書き込むことではありません。1番目のブロックを分割されて出たから、対応の格納アドレスと副本アドレスを獲得して順序で各DataNodeにデータを発送したまでは、一つの連続、完全のアップロード流れです。その初めて、次の2番目のブロックのアップロード流れを始めて進行します。つまり、上図の緑部分が一回のアップロード流れを表すだけ。
...
**step4/step5**：今1番目の図を参考としてDate Queueに複数のDataNode節点アドレスがあり、分別がdn16、dn57、dn89三つの節点です。クライアントのFSDataOutputStreamが最前のdn16に信号を発送して、次dn16節点から2番目のdn57節点に知らせ、最後のdn89節点まで受信した済みです。その過程に節点ごとも逐次にクライアントに通知してDate Queueにの各節点が正常だと確認させます。そこまで一つの完全な通信チャンネルを確立できます。ナンバー⑧とナンバー⑨が組み合わせて回路みたいものがパイプライン（Pipeline）と呼ばれます（⑥）。

　　次は正式にブロックのアップロードが開始します。読み込みと同じでブロックがパケット（packets）に分割される形式で一つ一つを目標のDataNodeに発送します。

　　dn16節点へ発送してのパケットを先にメモリに格納しておきます。格納した後、次のdn57節点にパケットを発送し、最後のdn89節点がパケットの受け取りが済むなら

逐次にクライアントに成功の確認情報を返却します。完全のブロックパケットを受信した限り、ブロックを永続化にしてファイルに書き込みます（多分）。最終にクライアントのDFSOutputStreamが返却信号を受信したまで一つの完全のデータストリート伝送が形成されます。残りの過程が上述の流れを繰り返すだけです。
