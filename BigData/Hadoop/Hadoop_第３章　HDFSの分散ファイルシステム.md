...
step2：上図には不足点があるのはクライアントがNameNodeに請求を発出してファイルの分割ブロックの格納アドレス情報を獲得する矢印を付くはずで、非常に重要の過程です。一つ目のブロックを分割されて出たら対応の格納アドレスを獲得し、副本も含まれて全てのブロックをアップロードしたまでは一つの連続、完全のアップロード流れです。その後で次の二つ目のブロックのアップロード流れを始めて進行するわけです。つまり、図の下の緑部分が一回のアップロード流れに属するだけです。

step3：一つ目のブロック情報を獲得してFSDataOutputStreamインスタンスを作成し、読み込みのFSDataInputStreamに比べて明らかに出力の用途です。NameNodeから取得した格納情報に従って対応のDataNodeに通信を立てます。

step4/step5：図を参考として説明してきて今は複数のDataNode節点アドレスを返却されてあり、分別がdn16、dn57、dn89三つの節点です。クライアントのFSDataOutputStreamが一つ目のdn16に信号を発送してアップロード操作があると知らせます。次にdn16節点がdn57節点に知らせ、次第に次の節点に同じな操作を行います。最後のdn89節点まで請求信号を受信したら、前の節点に始めて返信するになります。最初の送信者FSDataOutputStreamが順調にdn16節点の返信を受け取ると一つの完全な通信チャンネルを確立します。もちろんその途中である節点が故障になったらクライアントが改めてNameNodeに新たなアドレスを返信させます。

　　次は正式にブロックのアップロードが開始します。読み込みと同じでDFSOutputStreamはデータを書き込む際にはデータブロックを一つ一つのパケット（packets）に分割される形式で目標のDataNodeに発送します。dn16節点へパケットを発送してdn16節点が受け取るパケットをまずメモリに格納しておきます。格納済みに次のdn57節点へパケットをアップロードします。最後のdn89節点がパケットの受け取りが済むなら逐次に上の節点に完成信号を返却して、クライアントのDFSOutputStreamが返却信号を受け取るまで一つの完全のデータチャネルが完成しました。



パイプライン形式で送信します。これらのパケットは内部のデータキュー（data queue）に書き込まれます。データキューはNameNodeにデータレプリカを保存するのに適したDataNodeのリストを要求し、これらのDataNodeがパイプラインデータストリームを形成します。レプリカ因子が3の場合、このデータストリームには3つのDataNodeが含まれます。
