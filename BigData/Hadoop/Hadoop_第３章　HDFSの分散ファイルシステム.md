...
step2：NameNodeに請求を発出して必要の情報を返却されます。この必要の情報がブロックの基本情報、ブロックのアドレス、対応の副本アドレスなど含まれます。若しブロック数量が多すぎで、分割に一部分のブロック情報を先に返却されるという方式を選択する可能性もあります。もちろん、最後に完全の目標ファイルを継ぎ合わせるためにブロック情報の返却が一定の順序で並べます。
