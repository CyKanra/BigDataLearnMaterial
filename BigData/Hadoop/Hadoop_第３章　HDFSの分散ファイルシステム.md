...
**step3**：アップロード許可を獲得した後でFSDataOutputStreamインスタンスを作成され、200M目標ファイルを分割すると始めます。

　　書き込み流れが一括に書き込むことではありません。1番目のブロックを分割されて出たから、対応の格納アドレスと副本アドレスを獲得して順序で各DataNodeにデータを発送したまでは、一つの連続、完全のアップロード流れです。その初めて、次の2番目のブロックのアップロード流れを始めて進行します。つまり、上図の緑部分が一回のアップロード流れを表すだけ。パイプライン（Pipeline）

![061114_0923_LearnHDFSAB2](D:\OneDrive\picture\Typora\BigData\Hadoop\061114_0923_LearnHDFSAB2.webp)

　　NameNodeから取得したDataNodeアドレスのリストをData Queueにして格納します（④）。そのあるブロックにとって何のDataNodeに格納されておくのアドレス情報がDataStreamerから担当してNameNodeから獲得します（⑤）。

**step4/step5**：１番目の図を参考としてDate Queueに複数のDataNode節点アドレスがあり、分別がdn16、dn57、dn89三つの節点です。クライアントのFSDataOutputStreamが最前のdn16に信号を発送してアップロード操作があると知らせます。次にdn16節点からdn57節点に知らせ、最後のdn89節点受信したら前の節点に返信を始めます。最初の送信者FSDataOutputStreamが順調にdn16節点の返信を受け取ると一つの完全な通信チャンネルを確立できます。ナンバー⑧とナンバー⑨が組み合わせて回路みたいものがパイプライン（Pipeline）と呼ばれます（⑥）。もちろんその途中である節点が故障になったら改めてNameNodeに新たなアドレスを請求します。
