...
step2：上図には不足点があるのはクライアントがNameNodeに請求を発出してファイルの分割ブロックの格納アドレス情報を獲得する矢印を付くはずで、非常に重要の過程です。一つ目のブロックを分割されて出たら対応の格納アドレスを獲得し、副本も含まれて全てのブロックをアップロードしたまでは一つの連続、完全のアップロード流れです。その後で次の二つ目のブロックのアップロード流れを始めて進行するものです。つまり、図の下の緑部分が一回のアップロード流れに属するだけです。

step3：一つ目のブロック情報を獲得してFSDataOutputStreamインスタンスを作成し、読み込みのFSDataInputStreamに比べて明らかに出力の用途です。
