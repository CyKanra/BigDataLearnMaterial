...
　　下の図はあるファイルを読み込んでダウンロードするの流れです。仮に読み込みの目標ファイルが200Mサイズにして、128Mと72Mに分割されて少なくとも二つのブロックで格納されました。次はHDFSの具体の読み込み流れを説明しましょう。
...
step1：クライアントが先ずDistributed FileSystemのインスタンスを作成し、NameNodeに請求を発出して目標データのブロック情報やアドレスなどを取得する作用があります。

step2：NameNodeに請求を発出して必要の情報を返却されます。それがブロックの基本情報、ブロックのアドレス、対応の副本アドレスなど含まれます。若しブロック数量が多すぎで、分割に返却される方式を選択する可能性があります。以上の重要な情報が全てメタデータに属してNameNodeに管理されます。
