...
- バッファへ転送する時に分類処理がある。それはデータのkeyをハッシュ化（Hash）し、そのKeyをMapTask数で割って余りを求める。同じ余りデータが一緒に並べられる。
- その結果は、1つのMapTaskから出力された同じ余りを持つ`<key：単語/value：単語数>`ペアが、この段階で纏められてパーティションになる。これは、MapReduce処理負荷を均等化するための重要なステップでもある。
- その一時キャッシュが環状バッファと呼ばれる。本質には2つ配列から構成されており、1つ目の配列には`<key：単語/value：出現回数>`ペアを格納する。2つ目の配列にはそれらのペアに対応する索引が格納されている。図の1つ目に示されている、2つの半円形の矢印が組み合わさったような記号が、この環状構造を表している。
