...
　　RCFile（Record Columnar File）は、SequenceFileに類似したキーと値のデータファイルです。RCFileは行指向型と列指向型の利点と欠点を組み合わせて混合格納形式です。

　　RCFileは、先ずデータを行単位で水平に分割し、これにより1行のデータは同じクラスタ節点（node）に保存されることが保証できる。次に、行を垂直に分割して列指向型形式で保存される。

![image-20230913153222111](C:\Users\Izaya\AppData\Roaming\Typora\typora-user-images\image-20230913153222111.png)

**ORCFILE**

　　ORC File（Optimized Row Columnar File）は、RCFileを最適化したもので、Hive 0.11で導入されたファイルフォーマットです。このファイルフォーマットは、Hiveデータを効率的に格納する方法を提供し、他のフォーマットの不足を克服することを目指して設計されています。ORC Fileを使用すると、Hiveのデータの読み取り、書き込み、および処理の性能を向上させることができます。

ORCファイルの構造は、次の3つの主要な部分から成り立っています：

1. ファイルフッター（File Footer）：ファイル内のストライプのリスト、各ストライプの行数、および各列のデータ型などが含まれています。また、各列の最大値、最小値、行数、合計などの情報も含まれています。
2. ポストスクリプト（Postscript）：圧縮パラメータと圧縮サイズに関する情報が含まれています。
3. ストライプ（Stripe）：実際のデータを格納する場所で、通常はデフォルトで1つのストライプのサイズは250MBです。ストライプ内のデータは列ごとにエンコードされ、複数のストリームに分割されて格納されます。また、ストライプフッターにはストライプのメタデータ情報も含まれています。
