...
- **id**：当のサーバーのid番号、配置ファイルにmyidで表示し、も上記のServerIDです。
...
　　起動直後の状態であり、各サーバーは自身をLeaderとして自動的に選出し、投票を行う。投票の過程は選挙で必要な基本的な要素が含まれる。例えば、myidが1で、ZXIDが0の場合、(1,0)と表記する。各サーバーは自身をLeaderとして優先的に選出するため、server1の投票は(1,0)で、server2の投票は(2,0)となり、後ろのサーバーが同様の方式にする。
...
　　初期投票を送信した後、受け取ったサーバーが投票情報の処理が開始する。
...
　　外部投票と現在のサービスインスタンスの選挙回数が同じ場合、投票同士の比較操作が必要になります。
...
　　Voteクラスの定義（apache.zookeeper.server.quorum.Vote）は既に知っています。初期化段階では、各サーバーは自身をリーダーとして推薦し、自身を主とした選挙票を初期化します。

**初期化された選挙票の送信**

選挙票を初期化したら、自身の選挙票情報をsendQueueキューに保存し、各SIDに対応するworkerSenderがそれを送信します。

1. **外部からの投票情報の受信**

初期化段階では、自身の選挙票情報を送信するだけでなく、他のサービスインスタンスからの選挙票情報も受信します。これらの情報はrecvQueueキューに保存され、他の選挙票情報を取得できない場合は、現在のサービスインスタンスが他のサービスインスタンスと接続を維持しているか確認します。接続が切断されたか、接続がない場合、再度接続を確立します。ただし、ここでも接続の確立は現在のサービスのSIDを使用して行われ、重複した接続の作成を防ぎます。

1. **選挙回数の判断**

初期選挙票を送信した後、他のサービスインスタンスから受信した選挙票情報を処理し始めます。最初に受信した外部投票の選挙回数が現在の選挙回数より大きいかどうかを判断します。

- もし外部投票の選挙回数が現在のサービスインスタンスの選挙回数より大きい場合、現在のlogicalclockを更新し、受信したすべての選挙票情報をクリアし、選挙票と外部投票を再度比較して自身の選挙票情報を変更し、再度送信します。
- もし外部投票の選挙回数が現在のサービスインスタンスの選挙回数より小さい場合、その選挙票情報は無視し、自身の選挙票情報を再度送信します。
- もし外部投票の選挙回数と現在のサービスインスタンスの選挙回数が一致する場合、選挙票の内容を比較する必要があります。

1. **選挙票の比較**

選挙票の比較は、選挙アルゴリズムの中心的な論理です。これはFastLeaderElection#totalOrderPredicateメソッドで実装されており、現在のサービスインスタンスの選挙票情報を変更して新しい選挙票情報を再度送信する必要があるかを確認するために行われます。選挙回数、ZXID、SIDの順に比較します。

- もし外部投票の選挙回数が現在のサービスインスタンスの選挙回数より大きい場合、選挙票を変更する必要があります。
- もし選挙回数が同じであれば、ZXIDを比較します。外部投票のZXIDが大きい場合、現在のサービスインスタンスの選挙票情報を変更する必要があります。
- もしZXIDも同じであれば、SIDを比較します。外部投票のSIDが大きい場合、現在のサービスインスタンスの選挙票情報を変更する必要があります。
...
各サービスインスタンスは、送信や受信の過程で、選挙票情報をクリアまたは無視するかどうかに関係なく、各選挙票の情報をrecvSetに保存し、統計計算を行います。SIDで選挙票情報を区別して内部ストアし、統計を行います。半数以上のサービスインスタンスが投票したことがわかれば、選挙票情報の再送信を停止し、選挙を終了します。それ以外の場合は、上記の送信と受信の選挙票プロセスを繰り返します。

1. **Leaderの選出後、サービスインスタンスの状態変更**

投票が停止された後、最終的なLeaderサービスインスタンスが統計されると、各サービスインスタンスの状態を変更し始めます。具体的なロジックは、選ばれたLeaderが自身かどうかをチェックし、Leaderが自身である場合は状態をLEADINGに変更します。Leaderでない場合は、状態をFOLLOWINGまたはOBSERVINGに変更します。
