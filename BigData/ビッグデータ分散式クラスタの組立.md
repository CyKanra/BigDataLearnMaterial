...
　　パスワード認証にのパスワードはCentOSのマウント流れにrootユーザーに対して設定されたのパスワードで、SSHを使うかどうか関係ない。つまり、「パスワード不要の登録」効果にはSSHパスワード認証の使いによって至らない。

　　次、後者の認証方式はSSH作成しての公開鍵や秘密鍵を一つの鍵ペアに組み合わせるかどうかと判断する過程です。例えば、Centos1で作成された鍵ペアが公開鍵や秘密鍵二つの部分になり、公開鍵を目標リモートシステムのあるファイルに書き込み、Centos1サーバーからコマンドやファイルとかを転送する時に自分の秘密鍵や相手に置いての公開鍵が一組にするかどうかから、Centos1がそのサーバーに訪問の許可があるかどうかを判断してくる。

　　その過程で特別ユーザーやパスワードの入力が必要ない、且つ、

先ず、SSH認証方式を選んでクラスタの間に通信方式として行い、後、SSHのパスワード認証の方式は安全性が高くない、一方で毎回通信を立てて暗証番号が要る流れが、何とか安定性が足りないと思う。そして、ここで暗証番号不要で公開鍵認証を選んで、完全にサーバー上のファイルの公開鍵や秘密鍵が共同に認証し、特別に暗証番号を入力の操作が要らないという方式に決まることになる。
