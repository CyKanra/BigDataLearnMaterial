...
　　その過程は身元証明書の確認みたい、来訪者の身元証明書が目標サーバーに預かった情報に適当したらアクセスを許可できる、ユーザーやパスワードの入力が必要ない。そのために、クラスタの間に通信が効率と利便性を向上られ、特別に自動化で実行する必要があるタスクにとって不可欠です。今は、パスワード不要の登録の設定を始める。
