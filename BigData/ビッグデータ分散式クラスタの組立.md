...
　　Linux01仮想マシンの設定画面に入って、イメージファイルを選択する。OSのバージョンは「CentOS-7-x86_64-DVD-1810.iso」を選択し、互換性を考えて、Hadoop、HBase、Zoookeeperなどソフトウェアがそのバージョン下に運行できると確認しました。できるだけこのバージョンを使用して、不明のエラーを避けられます。
...
　　Liunⅹ01仮想マシンを起動し、第二の図で表れて「Install CentOS 7」を選択してOSに入って、Centosのインストールを始めます。
...
　　rootユーザーを使用のが十分で他の新ユーザーを作成しません。後は、boot分配大小が足りないなら、このインストール画面に止まて進められないという情況に遭ってあります。
...
　　ネットワーク配置する前にちょっとVMwareのネットワーク変数を確認しておき、仮想マシンには任意のIPアドレスを書き込むのが駄目です。
...
　　改めてネットワーク設定を示して、新しいIPアドレス情報が書き込みました。

![image-20240201142836164](D:\OneDrive\picture\Typora\BigData\Preparatory Work\image-20240201142836164.png)

#### 基本環境設定

　　ネットワークを設定してIPアドレスを決めてから、外部からOSシステムに通信することができる。ここでSecureCRT工具を選んでVMwareに代わってOSに訪問します。リモート訪問、ファイル転送、コーヒーなど操作がSecureCRTには中々便利である。以下はSecureCRTで登録の流れを表す。
...
　　この部分は、SSH知識にかかわってあるが、深く了解していっては必要ない、せめて以後のビッグデータ学習に邪魔することがない。一言で言えば、SSH（Secure Shell）はサーバー間の通信の認証や暗号化など働きを提供し、安全性を保証できるの転送手段です。

　　SSHの通信方式は二つに分けて、パスワード認証や公開鍵認証である。

　　前者に主な使い場合はリモートサーバーを訪問する。例えば、SecureCRT工具がSSH方式によってリモートサーバーへ接続する選択肢がある。又は、コマンドライン画面に「ssh root@192.168.31.136」を入力して他のサーバに登録もそういうことです。
...
　　その過程は身元証明書の確認みたい、来訪者の身元証明書が目標サーバーに預かった情報に適当してのみアクセスを許可し、ユーザーやパスワードの入力などが必要ない。そのために、クラスタの間に通信が効率と利便性を向上られ、特別に自動化で実行する必要があるタスクにとって不可欠です。次は、公開鍵認証の設定を始める。
...
　　SSHパスワードを生成するコマンドで、-t引数は生成するアルゴリズムを示し、rsaとdsaの二種類があり、-P引数はパスワードを使用して鍵ペアの作成を補助する動きです。""（空文字列）を書き込むとはパスワードなしの意味です。

　　今回は特別にSSHパスワードを付加してみて、検証する時パスワード不要の登録は下の図のように実現できない、依然にパスワードを必ず入力し、この点を注意しておく。
...
　　centos2へ伝送するファイルが成功かどうか確認し、問題ないなら、今はcentos1とcnetos2間にパスワード認証不要の動きもう実現した。
...
- centos2のid_rsa.pub公開鍵をauthorized_keysファイルに書き込む
...
　　centos2のauthorized_keysファイルに2つの公開鍵が保留する。ここまで一つの完全の流れであり、次のサーバーに伝送して上の操作ように行う。

　　全て終わりなら、最後のcentos4のauthorized_keysが理論的に4つの公開鍵段落が保留される。そのauthorized_keysファイルを他のサーバーに発送して元のファイルを上書く、サーバー毎にのauthorized_keysファイルは4つの公開鍵が保留してあることが保証する。
...
　　任意にテストファイルを作成して他のサーバーに伝送する。最初でパスワード、確認メッセージなどあるが、後のが直接にパスワード不要に接続して任務を実行できる。
...
　　時間を同期することはクラスタ内の全ての節点の時刻が同じになり、 主な節点が外部の時刻に参照し、各節点がその主な節点に参照すると時刻の同期を実現する。

　　その外部の時刻はネットワークインフラストラクチャ（network infrastructure ）上に構築された公開NTP時刻同期サービスから現在時刻を提供し、例えば、Amazon Time Sync ServiceとGoogle Public NTP。

　　NTP（Network Time Protocol）とはネットワークを通じて現在時刻の情報を送受信する通信規約の一つ。時刻情報を配信するサーバーと時刻合わせを行うクライアント間、及びサーバー間の通信方法を定めている。常に使用するNTPのネットアドレスがAmazon Time Sync Service：time.aws.comとGoogle Public NTP：time.google.comである。
