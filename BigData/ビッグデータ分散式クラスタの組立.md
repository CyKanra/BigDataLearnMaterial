...
　　ネットワーク配置する前にちょっとVMwareのネットワーク変数を確認しておき、仮想マシンには任意のIPアドレスに書き込むのが駄目です。
...
　　OSシステムのコマンド画面に入って「ip addr」を入力してIPアドレスに関する情報が欠けることが見えます。従って、目下の任務は手動でシステム配置ファイルを修正し、サーバを外部のネットに接続させます。
...
　　ビッグデータシステムは一般的に企業内部に使用するのが存在で、クラスタ間に通信の安全性を考えることが少ない、firewallとselinuxなど安全保障を停止にしてエラーを避けられます。
