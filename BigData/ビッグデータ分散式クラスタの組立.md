...
　　時間の同期はクラスタ内の全ての節点の時刻が同じになることをずっと保証します。一つの節点を時刻サーバとして外部の時刻に参照し、他はその時間サーバをもって時刻の同期を進行します。
...
　　NTP（Network Time Protocol）とはネットワークを通じて目標の時刻情報を取得しての通信規約です。主な使い場合は時間情報の交換によって遅延があるコンピュータ間で正確な時刻に同期させます。常に使用するNTPのネットアドレスがAmazon Time Sync Service：time.aws.com、Google Public NTP：time.google.comと日本標準時：ntp.nict.jpであります。
