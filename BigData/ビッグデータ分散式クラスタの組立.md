...
　　残りの3つ仮想マシンを起動し、サーバごとに応じてネットワーク配置を改めて設定します。
...
　　他のCentos3、Centos4は次第に上記のように変更を行い、ここて重複の流れを省きます。
...
　　今回は特にSSHパスワードを付加してみて、パスワードの入力が必要で下の図のようにパスワード不要が実現できません。
...
- 中にの一つのサーバ（centos1）について以下操作を行い

```
#.sshに入って
cd ~/.ssh

#id_rsa.pub中に内容をauthorized_keysファイルに書き込み
